...
function checkForm() {
    ...
    if (!apply_opt1.checked || !apply_opt2.checked || !apply_opt3.checked ||
        !apply_opt4.checked || !apply_opt5.checked) {
        var txt = document.getElementById("err_aplly");
        txt.textContent = "필수 항목에 모두 동의해주세요.";
        txt.className = "redTxt";
        return false;
...
/* 전체 체크 하기 */
function checkApply(checkbox) {
    const boxed = document.getElementsByName("ck");
    boxed.forEach((checkbox) => {
        checkbox.checked = apply_all.checked;
        if (checkbox.unchecked) {
            document.querySelector("input[name=ck_all]").removeAttribute('checked');
        }
    })
}

function unCheckApply() {
    // 전체 체크박스
    const checkboxes = document.querySelectorAll('input[name="ck"]');
    // 선택된 체크박스
    const checked = document.querySelectorAll('input[name="ck"]:checked');
    // select all 체크박스
    const selectAll = document.querySelector('input[name="ck_all"]');
